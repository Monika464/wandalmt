...
discountSchema.methods.calculateDiscount = function (amount, productId) {
    if (!this.isValid())
        return 0;
...
discountSchema.methods.useDiscount = async function (userId, orderId, discountAmount) {
    this.usedCount += 1;
    this.usageHistory.push({
        userId: typeof userId === "string" ? new Types.ObjectId(userId) : userId,
        orderId: typeof orderId === "string" ? new Types.ObjectId(orderId) : orderId,
        usedAt: new Date(),
        discountAmount,
    });
    await this.save();
