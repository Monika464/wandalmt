        ...
        if (videoId && mongoose.Types.ObjectId.isValid(videoId)) {
            ...
            if (video) {
                ...
                if (video) {
                    ...
                    console.warn(`Video with bunnyGuid ${bunnyVideoId} not found in database, but chapter will be saved`);
                }
            }
            const newChapter = {
            ...
            const chapterObject = savedChapter.toObject
...
export const editChapter = async (req, res) => {
    try {
        ...
        if (updateData.bunnyVideoId !== undefined) {
            if (updateData.bunnyVideoId === "" || updateData.bunnyVideoId === null) {
                ...
                if (video) {
                    ...
                    console.warn(`Video with bunnyGuid ${updateData.bunnyVideoId} not found`);
                    chapter.bunnyVideoId = updateData.bunnyVideoId;
                    chapter.videoId = undefined;
                }
            }
        ...
        if (updateData.number !== undefined)
            chapter.number = updateData.number;
        if (updateData.title !== undefined)
            chapter.title = updateData.title;
...
export const deleteChapterVideo = async (req, res) => {
    try {
        ...
        try {
            await axios.delete(`https://video.bunnycdn.com/library/${BUNNY_LIBRARY_ID}/videos/${bunnyVideoId}`, {
                headers: {
...
export const getChapterWithVideo = async (req, res) => {
    try {
        const { id, chapterId } = req.params;
        const resource = await Resource.findById(id);
        if (!resource) {
...
export const deleteChapter = async (req, res) => {
    try {
        ...
        if (chapter.bunnyVideoId) {
            try {
                await axios.delete(`https://video.bunnycdn.com/library/${BUNNY_LIBRARY_ID}/videos/${chapter.bunnyVideoId}`, {
                    headers: {
                        AccessKey: BUNNY_API_KEY,
                        Accept: "application/json",
                    },
...
export const setChapterVideo = async (req, res) => {
    try {
        const { id, chapterId } = req.params;
        ...
        if (!video) {
            console.warn(`Video with bunnyGuid ${bunnyVideoId} not found in database`);
        }
        // Ustaw pola
