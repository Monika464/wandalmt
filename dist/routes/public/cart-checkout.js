...
router.post("/cart-checkout-session", userAuth, async (req, res) => {
    try {
        ...
        sessionConfig.custom_text = {
            submit: {
                message: "Dziękujemy za zakupy! Dostęp do kursów otrzymasz natychmiast po płatności.",
            },
        };
...
router.get("/cart-session-status", userAuth, async (req, res) => {
    ...
    try {
        ...
        if (orderId) {
            ...
            console.log(`🔍 Looking for order by orderId: ${orderId}, found: ${order ? "yes" : "no"}`);
        }
        // Jeśli nie znaleziono, szukaj po stripeSessionId
        if (!order && session.id) {
            order = await Order.findOne({ stripeSessionId: session.id });
            console.log(`🔍 Looking for order by stripeSessionId: ${session.id}, found: ${order ? "yes" : "no"}`);
        }
        ...
        if (!order && session.metadata?.orderId) {
            ...
            console.log(`🔍 Looking for order by metadata.orderId: ${session.metadata.orderId}, found: ${order ? "yes" : "no"}`);
        }
        ...
        if (order) {
            ...
            if (session.invoice) {
                ...
                console.log(`🧾 Invoice created: ${order.invoiceId}, Number: ${invoice.number}`);
            }
            ...
            if (session.invoice) {
                console.log("Invoice object keys:", Object.keys(session.invoice));
                console.log("Invoice object:", JSON.stringify(session.invoice, null, 2));
            }
            // DODAJ FAKTURĘ JEŚLI ISTNIEJE
            if (session.invoice) {
                const invoice = session.invoice;
                ...
                console.log(`🧾 Invoice created: ${newOrder.invoiceId}, Number: ${invoice.number}`);
            }
            await newOrder.save();
            order = newOrder;
            console.log(`✅ Created new order ${order._id} from cart-session-status`);
        }
        ...
        if (order && order.products.length > 0 && order.user.userId) {
            ...
            if (productIds.length > 0) {
                ...
                if (resources.length > 0) {
                    ...
                    console.log(`🔹 ${resources.length} resources assigned to user ${order.user.userId}`);
                }
            }
        }
        ...
        setTimeout(async () => {
            try {
                console.log(`📧 Preparing to send confirmation email for order ${order._id}`);
                ...
                // Wywołaj endpoint
                const response = await fetch("http://localhost:3000/api/email/send-order-confirmation", {
                    method: "POST",
