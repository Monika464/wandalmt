...
import { changeEmail, requestPasswordReset, resetPassword, validateResetToken, } from "controllers/authController.js";
const router = Router();
//Login admin or user
...
router.post("/refresh-token", userAuth, async (req, res) => {
    try {
        if (!req.user || !req.token) {
