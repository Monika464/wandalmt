...
router.post("/", adminAuth, async (req, res) => {
    try {
        const { name, code, type, value, minPurchaseAmount, maxDiscountAmount, maxUses, userId, productId, validFrom, validUntil, isActive, } = req.body;
        // Walidacja
...
router.get("/:id/usage", adminAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const discount = await Discount.findById(id)
            .populate({
...
router.get("/report/summary", adminAuth, async (req, res) => {
    try {
